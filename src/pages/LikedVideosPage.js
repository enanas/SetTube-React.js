...
export default class LikedVideosPage extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    if (this.props.user != null) {
      UserService.getLikedVideos(this.props.user.id).then(
        (res) => {
  ...
  render() {
    return this.props.user === null ? (
      <Container textAlign="center" style={{ marginTop: "15%" }}>
        <Header inverted>
          You must be logged-in to check your liked videos!
        </Header>
    ...
    ) : this.props.user.likedVideos.length === 0 ? (
      <Container textAlign="center" style={{ marginTop: "15%" }}>
        <Header inverted>
          You have no liked videos. Like some videos first!
        </Header>
      </Container>
    ) : (
        <VideoLoader
